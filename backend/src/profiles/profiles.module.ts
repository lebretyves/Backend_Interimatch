...
import { interval, Professional, match } from "../domain/matching";
...
export class ProfilesService {
  ...
  async update(actor: string, b: ProfileDto) {
    ...
    return this.db.transaction(async (em) => {
      ...
      await em.query(
        "UPDATE profile SET display_name=$2,qualifications=$3,skills=$4,experience=$5,available=$6,unavailable=$7,latitude=$8,longitude=$9,radius_km=$10,accepted_shifts=$11,preferred_shifts=$12,visible=$13,updated_at=now() WHERE user_id=$1",
        [
          actor,
          b.displayName,
          b.qualifications,
...
class ProfilesController {
  ...
  @Get() async get(@Req() req: Request) {
    return this.db.transaction(async (em) => nurse(em, user(req)));
  }
