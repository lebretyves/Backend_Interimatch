...
const root=resolve(import.meta.dirname,'..');const files=execFileSync('git',['ls-files','-z'],{cwd:root,encoding:'utf8'}).split('\0').filter(Boolean);
...
const bad=files.filter(f=>{const data=readFileSync(resolve(root,f));return secrets.some(s=>data.includes(Buffer.from(s)));});
